...
def before_sarima():
    ...
    df_before_sarima = df_calender_rest[
        [
            "clinic_id",
            "clinic_name",
            "date",
    ...
    df_before_sarima = df_before_sarima.rename(
        columns={
...
def sarima_output():
    ...
    for clinic_id in df["clinic_id"].unique():
        ...
        df_clinic["national_holiday"] = df_clinic["national_holiday"].map(
            lambda x: 1 if x else 0
        )
        df_clinic["clinic_holiday"] = df_clinic["clinic_holiday"].map(
            lambda x: 1 if x else 0
        )
