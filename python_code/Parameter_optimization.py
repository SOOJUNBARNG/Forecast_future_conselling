...
def arima_objective(trial):
    ...
    current_date = pd.to_datetime(datetime.today().date())  # Convert current_date to Timestamp
    ...
    df_exog[["national_holiday", "clinic_holiday"]] = df_exog[["national_holiday", "clinic_holiday"]].apply(lambda x: x.astype(int))
    ...
    exog = df_exog.loc[df_exog.index <= current_date, ["national_holiday", "clinic_holiday", "day_of_week"]]
    ...
    try:
        # Fit ARIMA model
        model = ARIMA(y, order=(p, d, q), exog=exog, enforce_stationarity=False, enforce_invertibility=False)
        result = model.fit()
