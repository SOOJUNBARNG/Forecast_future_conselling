...
df_calender = df_calender[
    ["clinic_name", "date", "counseled", "holiday_flag", "tcb_holiday_flag"]
]
...
for idx, row in df_calender.iterrows():
    ...
    if (
        row["tcb_holiday_flag"] is True
        or pd.isnull(row["counseled"])
        or row["counseled"] == 0
        or row["counseled"] == ""
    ):
...
df_calender[["holiday_flag", "tcb_holiday_flag"]] = df_calender[
    ["holiday_flag", "tcb_holiday_flag"]
...
forecast_exog = pd.DataFrame(
    ...
    index=pd.date_range(
        start=df_calender.index[-1] + pd.Timedelta(days=1), periods=14, freq="D"
    ),
)
